...
class ActionLayout:
    ...
    def __init__(
        self,
        token_dim: int = 64,
        include_task_space_hands: bool = False,
        include_grippers: bool = False,
    ):
        fields = []
        cursor = 0
        fields.append(ActionField("motion_token", cursor, cursor + token_dim))
        cursor += token_dim
        if include_task_space_hands:
            # xyz + continuous rotation-6D per hand; task-space targets are checked by IK before execution.
            for side in ["left", "right"]:
                fields.append(ActionField(f"{side}_ee_target", cursor, cursor + 9))
                cursor += 9
        if include_grippers:
            for side in ["left", "right"]:
                fields.append(ActionField(f"{side}_gripper", cursor, cursor + 1))
                cursor += 1
    ...
    def pack(
        ...
        for name, v in values.items():
            ...
            if v.shape != (*batch_shape, f.dim):
                raise ValueError(
                    f"{name}: expected {(*batch_shape, f.dim)}, got {tuple(v.shape)}"
                )
