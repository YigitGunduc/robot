...
def main():
    ap = argparse.ArgumentParser(description="Augment preprocessed BONES clips with MuJoCo body FK for SONIC body-space rewards")
    ap.add_argument("--motions", required=True)
    ap.add_argument("--mjcf", required=True)
    ap.add_argument("--bodies", nargs="*", help="Body names; default is every named non-world body")
    args = ap.parse_args()
    bodies = args.bodies or SONIC_TRACKED_BODY_NAMES
    files = sorted(p for p in Path(args.motions).rglob("*.npz") if p.name not in {"_manifest.npz", "_packed_metadata.npz"})
    for i, p in enumerate(files):
        clip = MotionClip.load_npz(p)
        augment_clip_with_mujoco_fk(clip, args.mjcf, bodies).save_npz(p)
        if i % 100 == 0:
