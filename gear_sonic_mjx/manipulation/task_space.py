...
class TaskSpaceSafetyGate:
    ...
    def clamp(self, target: EndEffectorTarget) -> EndEffectorTarget:
        pos = torch.maximum(torch.minimum(target.position, self.xyz_high.to(target.position)), self.xyz_low.to(target.position))
    ...
    def valid(self, target: EndEffectorTarget) -> torch.Tensor:
        finite = torch.isfinite(target.position).all(-1) & torch.isfinite(target.rotation_6d).all(-1)
        inside = ((target.position >= self.xyz_low.to(target.position)) & (target.position <= self.xyz_high.to(target.position))).all(-1)
