...
class BonesMotionLibrary:
    ...
    def __init__(self, root: str | Path, target_fps: float = 50.0, cache_size: int = 256):
        self.root = Path(root)
        ...
        if manifest.exists():
            ...
            duration = np.maximum(src_frames - 1, 0) / src_fps
            target_frames = np.maximum(2, np.rint(duration * self.target_fps).astype(np.int64) + 1)
            self.lengths = torch.from_numpy(target_frames).long()
        else:
            self.files = sorted(p for p in self.root.rglob("*.npz") if p.name != "_manifest.npz")
            if not self.files:
            ...
            self.lengths = torch.tensor([self._load(i).num_frames for i in range(len(self.files))], dtype=torch.long)
    ...
    def sample_future_numpy(self, motion_id: int, frame: int, num_future: int = 10, dt: float = 0.1) -> dict[str, np.ndarray]:
        clip = self._load(int(motion_id))
        stride = max(1, int(round(dt * clip.fps)))
    ...
    def batch_future(
        ...
        return {
            k: torch.as_tensor(np.stack([r[k] for r in rows]), dtype=torch.float32, device=device)
            for k in rows[0]
        }


    def batch_current(self, motion_ids: torch.Tensor, frames: torch.Tensor, device: torch.device | str) -> dict[str, torch.Tensor | None]:
        rows = []
        for m, f in zip(motion_ids.tolist(), frames.tolist()):
            clip = self._load(int(m)); i = min(int(f), clip.num_frames - 1)
            rows.append((clip, i))
        out: dict[str, torch.Tensor | None] = {
            "root_pos": torch.as_tensor(np.stack([c.root_pos[i] for c, i in rows]), dtype=torch.float32, device=device),
            "root_quat_wxyz": torch.as_tensor(np.stack([c.root_quat_wxyz[i] for c, i in rows]), dtype=torch.float32, device=device),
            "joint_pos": torch.as_tensor(np.stack([c.joint_pos[i] for c, i in rows]), dtype=torch.float32, device=device),
            "joint_vel": torch.as_tensor(np.stack([c.joint_vel[i] for c, i in rows]), dtype=torch.float32, device=device),
        }
        for key in ["body_pos", "body_quat_wxyz", "body_linvel", "body_angvel"]:
            if all(getattr(c, key) is not None for c, _ in rows):
                out[key] = torch.as_tensor(np.stack([getattr(c, key)[i] for c, i in rows]), dtype=torch.float32, device=device)
            else:
                out[key] = None
        return out

    def upper_body_mix(
        self, q_future: torch.Tensor, qd_future: torch.Tensor, p: float = 0.5, upper_start: int = 12
    ...
    def freeze_frame(self, q: torch.Tensor, qd: torch.Tensor, probability: float = 0.1) -> tuple[torch.Tensor, torch.Tensor]:
        # NVIDIA TrackingCommandCfg exposes freeze_frame_aug_prob=0.1.
        if probability <= 0:
            return q, qd
        mask = torch.rand(q.shape[0], device=q.device) < probability
        if mask.any():
            q = q.clone(); qd = qd.clone()
...
class PackedBonesMotionLibrary:
    ...
    def __init__(self, root: str | Path, target_fps: float = 50.0):
        ...
        self.body_names = tuple(str(x) for x in meta["body_names"].tolist()) if "body_names" in meta and len(meta["body_names"]) else None
        self.names = [str(x) for x in meta["names"].tolist()] if "names" in meta else [str(i) for i in range(len(self.lengths))]
        self.arrays: dict[str, np.ndarray] = {}
        for key in ["root_pos", "root_quat_wxyz", "joint_pos", "joint_vel", "body_pos", "body_quat_wxyz", "body_linvel", "body_angvel"]:
    ...
    def _global_current_indices(self, motion_ids: torch.Tensor, frames: torch.Tensor) -> np.ndarray:
    ...
    def batch_future(self, motion_ids: torch.Tensor, frames: torch.Tensor, num_future: int, dt: float, device: torch.device | str, frame_cap: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
        m = motion_ids.detach().cpu().numpy().astype(np.int64, copy=False)
        f = frames.detach().cpu().numpy().astype(np.int64, copy=False)
        stride = max(1, int(round(float(dt) * self.target_fps)))
        rel = f[:, None] + np.arange(num_future, dtype=np.int64)[None] * stride
        if frame_cap is not None:
            caps = frame_cap.detach().cpu().numpy().astype(np.int64, copy=False)[:, None]
    ...
    def batch_current(self, motion_ids: torch.Tensor, frames: torch.Tensor, device: torch.device | str) -> dict[str, torch.Tensor | None]:
        ...
        for key in ["body_pos", "body_quat_wxyz", "body_linvel", "body_angvel"]:
            out[key] = self._to_device(self.arrays[key][idx], device) if key in self.arrays else None
        return out

    def _load(self, idx: int) -> MotionClip:
        """Compatibility probe/small-clip path. PPO sampling does not use this method."""
        start = int(self.offsets_np[idx]); n = int(self.lengths[idx]); sl = slice(start, start + n)
        kwargs = {k: np.asarray(v[sl]) for k, v in self.arrays.items()}
        return MotionClip(
            name=self.names[idx], fps=self.target_fps,
            root_pos=kwargs["root_pos"], root_quat_wxyz=kwargs["root_quat_wxyz"],
            joint_pos=kwargs["joint_pos"], joint_vel=kwargs["joint_vel"],
            body_names=self.body_names,
            body_pos=kwargs.get("body_pos"), body_quat_wxyz=kwargs.get("body_quat_wxyz"),
            body_linvel=kwargs.get("body_linvel"), body_angvel=kwargs.get("body_angvel"),
        )

    def upper_body_mix(self, q_future: torch.Tensor, qd_future: torch.Tensor, p: float = 0.5, upper_start: int = 12) -> tuple[torch.Tensor, torch.Tensor]:
        if p <= 0 or q_future.shape[0] < 2:
            return q_future, qd_future
        b = q_future.shape[0]
        mask = torch.rand(b, device=q_future.device) < p
...
def open_motion_library(root: str | Path, target_fps: float = 50.0, cache_size: int = 256) -> BonesMotionLibrary | PackedBonesMotionLibrary:
    root = Path(root)
