...
class RolloutStorage:
    ...
    def minibatches(self, num_minibatches: int, epochs: int) -> Iterator[RolloutBatch]:
        ...
        if num_minibatches <= 0 or num_minibatches > total:
            raise ValueError("num_minibatches must be in [1, nsteps * nenv]")
        flat = lambda x: (
            x.reshape(total, *x.shape[2:]) if x.ndim > 2 else x.reshape(total)
        )
        tensors = [
            flat(x)
            for x in (
        ...
        for _ in range(epochs):
            ...
            for idx in torch.tensor_split(perm, num_minibatches):
...
class SonicActorCritic(nn.Module):
    ...
    def distribution(self, encoder_obs: torch.Tensor, proprio_obs: torch.Tensor):
        ...
        std = (
            self.log_std.exp()
            .clamp(self.std_min, self.std_max)
            .expand_as(out.action_mean)
        )
...
class PPOTrainer:
    ...
    def __init__(self, model: SonicActorCritic, cfg: PPOConfig):
        ...
        self.critic_opt = torch.optim.Adam(
            model.critic.parameters(), lr=cfg.critic_learning_rate
        )
    ...
    def update(self, storage: RolloutStorage) -> dict[str, float]:
        stats = {
            k: 0.0 for k in ["policy_loss", "value_loss", "entropy", "aux_loss", "kl"]
        }
        n = 0
        for mb in storage.minibatches(
            ...
            surrogate2 = -mb.advantages * torch.clamp(
            ...
            actor_loss = (
                ...
                + self.cfg.aux_reconstruction_coef * aux
            )

            self.actor_opt.zero_grad(set_to_none=True)
            actor_loss.backward()
            nn.utils.clip_grad_norm_(
                list(self.model.token_module.parameters()) + [self.model.log_std],
                self.cfg.max_grad_norm,
            )
            self.actor_opt.step()

            value = self.model.value(mb.critic_obs)
            value_clipped = mb.old_value + (value - mb.old_value).clamp(
            ...
            nn.utils.clip_grad_norm_(
                self.model.critic.parameters(), self.cfg.max_grad_norm
            )
